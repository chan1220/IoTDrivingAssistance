...
class obdex(QtCore.QThread):
	...
	on_changed_hbreak_count = QtCore.pyqtSignal(object, object)
	on_hard_accel           = QtCore.pyqtSignal(object, object)
	on_hard_rpm             = QtCore.pyqtSignal(object, object)
	...
	def _update_fuel_use(self):    # 총 기름 소모량 계산(1초에 1번 호출)
		...
		if self.fuel_use != 0:      # 평균 연비 계산
			...
			self.on_changed_avr_fuel.emit(self.avr_fuel)
	...
	def _update_hard_break(self):  # 급브레이크 계산 (1초에 1번 호출)
		...
		if (self.speed > 50) & ((self.prev_speed - self.speed) > 10):
			self.hard_break += 1
			self.on_changed_hbreak_count.emit(self.hard_break, self.speed)
	...
	def _update_hard_accl(self):   # 급가속 계산 (1초에 1번 호출)
		...
		if self.throttle > 40:
			self.hard_accel += 1
			self.on_hard_accel.emit(self.hard_accel, self.throttle)
	...
	def _update_hard_rpm(self):     # 고알피엠 계산 (1초에 1번 호출)
		...
		if self.rpm > 3000:
			self.hard_rpm += 1
			self.on_hard_rpm.emit(self.hard_rpm, self.rpm)
