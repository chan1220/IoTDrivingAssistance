...
class Worker(QThread):
    ...
    def insertDB(self):
        db = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='root', db='pidb', charset='utf8',
                             autocommit=True)  # DB 개방
        cursor = db.cursor()
        sql_query = "INSERT RECORD VALUES('%s','%s',%lf,%d,%d,%d,%d,%d,%lf,'%s')" % (
            self.get_mac(), self.m_startTime, self.m_dis / self.m_total_fuel, self.m_avr_speed, self.m_high_rpm,
            self.m_break,
            self.m_accel, self.m_score, self.m_dis, self.m_endTime)
